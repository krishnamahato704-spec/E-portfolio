...
export function validateFile(file) {
  const types={pdf:'application/pdf',jpg:'image/jpeg',jpeg:'image/jpeg',png:'image/png',webp:'image/webp',docx:'application/vnd.openxmlformats-officedocument.wordprocessingml.document',pptx:'application/vnd.openxmlformats-officedocument.presentationml.presentation'};
  const ext=file.name.split('.').pop().toLowerCase();
  if(!types[ext] || types[ext]!==file.type) throw new Error('Choose a PDF, JPG, PNG, WebP, Word (.docx) or PowerPoint (.pptx) file.');
...
export async function uploadFile(file,token) {
  ...
  await request('/storage/v1/object/portfolio-media/'+path,{method:'POST',token,headers:{'Content-Type':file.type,'x-upsert':'false'},body:file});
