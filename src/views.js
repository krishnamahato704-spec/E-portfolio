...
function profileSkillsSection(c){
  ...
  return `<section class="section compact profile-skills-section"><h2>Developing teaching practice.</h2><p>These are areas of preparation and practice. The published artifacts show the evidence currently available.</p><ul class="skill-pill-list sr-only">${c.competencies.map(skill=>`<li class="skill-pill">${esc(skill)}</li>`).join('')}</ul><div class="skill-cluster-grid">${clusters.map(cluster => `<article class="skill-cluster-card interactive-3d-card" data-motion="fade-up"><span class="skill-cluster-tag">${cluster.tag}</span><h3 class="skill-cluster-title">${cluster.title}</h3><ul class="skill-cluster-items">${cluster.items.map(item => `<li class="skill-cluster-item">${item}</li>`).join('')}</ul></article>`).join('')}</div></section>`;
}
...
export function view(route,c,base='./') {
 ...
 const observation=c.experiences.find(e=>e.id==='observation');
  ...
  case 'gallery':return `<div class="container gallery-page">${heading('06 / Gallery','Teaching &amp; Classroom Gallery.','Visual documentation of school internships, seminar presentations, classroom planning, and academic milestones.')}<div class="tlm-gallery-grid">${c.gallery.map(x=>`<article class="tlm-card interactive-3d-card"><div class="tlm-image-frame"><a class="archive-doc-frame" href="${esc(safeUrl(x.image))}" target="_blank" rel="noopener noreferrer" data-title="${esc(x.title)}" data-category="${esc(x.category||'GALLERY')}" data-caption="${esc(x.description||'')}">${img(x.image,x.title,base,'tlm-img')}<span class="doc-view-hint" aria-hidden="true">Examine record ↗</span></a></div><div class="tlm-body"><div class="tlm-meta-line"><span class="tlm-category-badge">${esc(x.category||'Document')}</span>${x.context?`<span class="meta-sep" aria-hidden="true">·</span><span class="tlm-context">${esc(x.context)}</span>`:''}</div><h3 class="tlm-title">${esc(x.title)}</h3>${x.description?`<p class="tlm-description">${esc(x.description)}</p>`:''}<div class="tlm-pedagogy"><strong>Inquiry context:</strong> Verified archival evidence connecting teacher education to classroom practice.</div></div></article>`).join('')}</div>${c.gallery.length?'':'<p class="empty-note">No gallery images are currently published. The teaching artifacts contain the available lesson evidence.</p>'}${link(base,'resources','View teaching artifacts')}</div>`;
