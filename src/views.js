...
const assetAliases = {
  ...
  'cert_gemini_educator': 'evidence/gemini-badge.webp',
  'cert_ncert_ai': 'evidence/diksha-ai.webp',
  'cert_nptel_writing': 'evidence/nptel-writing.webp',
  'cert_suraasa_linkedin': 'evidence/suraasa-linkedin.webp',
...
function img(url,alt,base,cls='',priority=false){const src=imageUrl(url,base);const size=dimensions[src?.match(/assets\/[^?#]+/)?.[0]];return src?`<img class="${cls}" src="${esc(src)}" alt="${esc(alt)}" ${size?`width="${size.width}" height="${size.height}"`:''} ${priority?'fetchpriority="high"':'loading="lazy"'} decoding="async">`:''}
