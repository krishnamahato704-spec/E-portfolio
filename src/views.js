...
export const routes={
  ...
  gallery:{path:'gallery/',title:'Visual Evidence',nav:'Gallery'},
...
export function header(route,base,c){
  const navItems = [
    ...
    {r:'gallery',num:'03',label:'Gallery'},
...
function resourceRows(c,base='./'){
  return c.resources.map((r,i)=>{
    const thumbSrc = r.thumbnail || r.image || (/\.(jpg|jpeg|png|webp)($|\?)/i.test(r.url) ? r.url : '');
...
function profileRecruiterSystem(c) {
  ...
  const col1 = [
    {
      ...
      value: 'History, Social Science &amp; English',
      note: 'Academic background in History &amp; Economics. English is an official B.Ed. pedagogy subject with internship teaching experience (does not hold a degree in English).'
    },
    {
      label: 'ROLE INTEREST',
      value: (p.roles && p.roles.length) ? p.roles.map(esc).join(' · ') : 'TGT Social Science · TGT English · PGT History',
      note: 'TGT Social Science and TGT English; PGT History upon completing M.A. History examinations in June 2027.'
    },
    {
      label: 'CURRENT STUDY',
      value: esc(studySummary(c)),
      note: inProgress
        ? 'Amity University (B.Ed. · Expected completion April 2027) and IGNOU (M.A. History · First year cleared · Expected completion June 2027).'
        : 'Formal academic qualifications documented in the education record below.'
    },
    {
      label: 'TARGET CLASSES',
      value: esc(p.targetClasses || 'Classes 6–10 (TGT) · Classes 11–12 (PGT History post-M.A. 2027)'),
      note: 'Middle school (Classes 6–8), secondary (Classes 9–10), and senior secondary History (Classes 11–12 post-M.A.).'
    },
    {
      label: 'AVAILABILITY',
      value: p.availability ? `Available from ${esc(p.availability)}` : 'Available from May 2027',
      note: 'Open to introductory recruitment discussions; available for interviews, school visits and demonstration lessons from February 2027.'
    }
  ];
  ...
  const col2 = [
    {
      label: 'LOCATION',
      value: p.location ? `Based in ${esc(p.location)}` : 'Based in Noida, India',
      note: 'National Capital Region (NCR).'
    },
    {
      label: 'RELOCATION',
      value: esc(p.workPreferences || 'Open to relocation anywhere in India'),
      note: 'Open to school teaching opportunities across all states and union territories in India.'
    },
    {
      label: 'ELIGIBILITY',
      value: esc(p.eligibility || 'CTET Paper II applied · Examination tentatively expected on 9 October 2026'),
      note: 'Applied for Central Teacher Eligibility Test (Paper II); examination tentatively expected on 9 October 2026.'
    },
    {
      label: 'BOARDS OF INTEREST',
      value: esc(p.targetBoards || 'Boards of interest: CBSE, ICSE, Cambridge and IB'),
      note: 'Curricular areas of interest for future teaching; not a claim of prior employment or certification under them.'
    },
    {
      label: 'LANGUAGES',
      value: (p.languages && p.languages.length) ? p.languages.map(esc).join(' · ') : 'English · Hindi · Nepali · Maithili',
      note: 'English, Hindi, Nepali (listening, speaking, reading, writing); Maithili (listening and reading).'
    }
  ];
...
export function view(route,c,base='./') {
 ...
 const observation=c.experiences.find(e=>e.id==='observation');
  ...
  case 'gallery': return `<div class="container gallery-page">${heading('Visual Evidence','Moments from practice.<br><em>The classroom in action.</em>','Visual documentation of school internships, community teaching, and educational workshops.')}
  <section class="section">
    <div class="placeholder-gallery-grid">
      <figure class="image-placeholder card-placeholder"><div class="placeholder-box"><span>Upload Gallery Image 1</span></div><figcaption>Add a descriptive caption here</figcaption></figure>
      <figure class="image-placeholder card-placeholder"><div class="placeholder-box"><span>Upload Gallery Image 2</span></div><figcaption>Add a descriptive caption here</figcaption></figure>
      <figure class="image-placeholder card-placeholder"><div class="placeholder-box"><span>Upload Gallery Image 3</span></div><figcaption>Add a descriptive caption here</figcaption></figure>
      <figure class="image-placeholder card-placeholder"><div class="placeholder-box"><span>Upload Gallery Image 4</span></div><figcaption>Add a descriptive caption here</figcaption></figure>
      <figure class="image-placeholder card-placeholder"><div class="placeholder-box"><span>Upload Gallery Image 5</span></div><figcaption>Add a descriptive caption here</figcaption></figure>
      <figure class="image-placeholder card-placeholder"><div class="placeholder-box"><span>Upload Gallery Image 6</span></div><figcaption>Add a descriptive caption here</figcaption></figure>
    </div>
  </section>
  </div>`;
