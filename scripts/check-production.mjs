...
try {
  ...
  const imageShapes = await page.locator('.gallery-thumb-img, .cert-img-thumbnail, .card-thumb-img, .featured-tlm-img').evaluateAll(images => images.map(img => {
    const box=img.getBoundingClientRect(), parent=img.parentElement.getBoundingClientRect();
    return {src:img.getAttribute('src'),ratio:Math.abs(box.width/box.height-img.naturalWidth/img.naturalHeight),clipped:parent.height+2<box.height};
  }));
  check('Evidence images retain source ratios without clipping',imageShapes.every(i=>i.ratio<.02&&!i.clipped),imageShapes.filter(i=>i.ratio>=.02||i.clipped));
  check('Original reports and PowerPoint decks are linked',await page.locator('.document-card').count()>=4 && await page.locator('.document-card a[href$=".pptx"]').count()===2);
  ...
  await broken.route('**/rest/v1/portfolio_public**',route=>route.fulfill({json:[{content:defaultContent,updated_at:'2026-09-24'}]}));
  await broken.goto(base); await broken.locator('img').evaluateAll(images=>images.forEach(img=>img.loading='eager')); await broken.locator('.image-unavailable').first().waitFor({state:'attached'});
